...
// * reducer - 我们传入的reducer纯函数，用来定义我们的state可以做的操作, 处理不同的 action type
...
export default function createStore(reducer, preloadedState, enhancer){
  ...
  if (typeof enhancer === 'function') {
    ...
    // eg: const enhancer = applyMiddleware(middleware1, middleware2);
    // enhancer 其实是 applyMiddleware 函数
  }

  let currentReducer = reducer  // 当前的reducer。Redux支持我们替换当前的reducer为另一个函数
  let currentState = preloadedState // 用来存储我们整个程序的状态
  let currentListeners = [] // 状态监听函数，当状态发生改变的时候会被调用。可以注册多个监听函数
  let isDispatching;
